...
@testset "Test straight line no current" begin
   ...
   lon2 = 10.0
   lat = 0.0
   dist, theta = SailRoute.haversine(lon1, lat, lon2, lat)
   wind_spd = 10.0
   wind_dir = 0.0
   curr_spd = 0.0
   curr_dir = 0.0
   bearing = SailRoute.min_angle(theta, wind_dir)
   vt = dist/SailRoute.perf_interp(circular_performance(), bearing, wind_spd)

   n_nodes = 7
   test_route = SailRoute.Route(lon1, lon2, lat, lat, n_nodes, n_nodes)
   ...
   # solve the route
   rs = SailRoute.route_solve(test_route, circular_performance(), start_time, times, grid_x, grid_y, wisp, widi, wadi, wahi, cusp, cudi)
   @test rs[1] ≈ vt
end
...
@testset "Straight line with current" begin
   ...
   curr_spd = 1.0
   curr_dir = 270.0
   v_t = dist/SailRoute.solve_speed_given_current(wind_spd, wind_dir, curr_spd, curr_dir, theta, circular_performance())
   n_nodes = 11
   test_route = SailRoute.Route(lon1, lon2, lat, lat, n_nodes, n_nodes)
